              ...
              <tbody className="divide-y divide-slate-200">
                {recentBooks.map((book) => (
                  <tr key={book.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-semibold text-red-600">₹{book.price}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${book.stock > 0 ? "bg-green-500" : "bg-red-500"}`}></div>
                        <span className={`text-sm font-medium ${book.stock > 0 ? "text-green-700" : "text-red-700"}`}>
                          {book.stock} units
                        </span>
                      </div>
                    </td>
